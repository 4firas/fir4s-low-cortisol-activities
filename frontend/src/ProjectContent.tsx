...
export const ProjectBody = ({ project }: { project: Project; }) => {
    ...
    return (
      <div className="my-8 tracking-wide">
        <PortableText
          ...
          components={{
            marks: { link: LinkMark },
            types: { quote: QuoteBlock, link: LinkBlock, vimeo: VimeoBlock, imageFigure: ImageBlock },
            block: { h2: H2Block, h3: H3Block, normal: PBlock },
          }}
        />
      </div>
    );
  ...
  // Fallback projects have `description` field
  const desc = (project as any)?.description ?? '';
  if (desc) {
    return <div className="my-8 tracking-wide"><P>{desc}</P></div>;
  }

  // Last resort: show subTitle
  if (project?.subTitle) {
    return <div className="my-8 tracking-wide"><P>{project.subTitle}</P></div>;
  }
