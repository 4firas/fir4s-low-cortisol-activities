...
export const DrawToRevealCanvas = ({
  ...
  const setUsingKeyboard = (value: boolean) => {
    ...
    if (value === false && keyboardUserPaintbrushRef.current) {
      // @ts-ignore
...
export const SkillArtWindow = ({
  ...
  useAddDrawFill(drawFills, skills[0], '#C39040', '#223434', breakpoint);
  useAddDrawFill(drawFills, skills[1], '#7F4D0F', '#BAB5B2', breakpoint);
  useAddDrawFill(drawFills, skills[2], '#BAB5B2', '#534328', breakpoint);
  useAddDrawFill(drawFills, skills[3], '#534328', '#BAB5B2', breakpoint);
  ...
  return (
    <>
        ...
        <div className="grid overflow-hidden absolute top-0 left-0 place-items-center w-full h-full">
          ...
          {showInstructions && (
            <div
              ...
              leading-[1] uppercase text-blueSlate
            ...
            {drawFills.map(([_drawFill, color], index) => (
              <button
                ...
                className={`grid place-items-center  border-black w-[3em] h-[3em]
                    ${currentFill === index ? 'border-[0.5em]' : 'border-[3px]'}
                ...
                aria-label={`Paint with ${color}-colored skill art`}
              />
            ))}
          </div>
        </div>
      ...
      {isFocusVisible && (
        <div className="absolute bottom-0 left-0 font-mono text-blueSlate bg-pitchBlack z-[5] p-[0.2em] text-[1em]">
          {"Don't use a mouse? "}
          <br />
