...
export function Computer() {
  ...
  const screenColorOn = useMemo(() => new Color(colors.darkGoldenrod), []);
  const screenColorOff = useMemo(() => new Color(colors.charcoalBlue), []);
  const bodyColor = useMemo(() => new Color(colors.blueSlate), []);
  const fillUmber = useMemo(() => new Color(colors.rawUmber), []);

  return (
    <animated.group
      scale={animatedComputerGroupScale}
      // @ts-expect-error spring-animated value compatible with R3F at runtime
    ...
    >
      ...
      <Scribble
        ...
        position={[-1, 0.7, 2.1]}
        lineWidth={0.6}
        color={computerOn ? screenColorOn : screenColorOff}
        rotation={[Math.PI, 0, -Math.PI / 15]}
      ...
      <CodeRings visible={computerOn} />
      {/* @ts-ignore drei attachArray type issue */}
      <Text
        position={[-1, 0.7, 2.1]}
